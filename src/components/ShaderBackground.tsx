...
const IMAGE_URL =
  "https://infogito.com/astra-blob/files/u_rg__f_hvac__1778516424705-2c9810e6f981/ChatGPT%20Image%20May%2011,%202026,%2012_17_10%20PM.png";
...
const ShaderPlane = ({
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      undefined,
      () => {
        // Image failed to load (proxy doesn't exist on Cloudflare Pages)
        // Use a procedural fallback texture so the shader always renders
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      // Update toggles
      materialRef.current.uniforms.tNegativeSpace.value = toggles.negativeSpace
        ? 1.0
        : 0.0;
      materialRef.current.uniforms.tAirflowStreaks.value =
        toggles.airflowStreaks ? 1.0 : 0.0;
      materialRef.current.uniforms.tConduit.value = toggles.conduit ? 1.0 : 0.0;
      materialRef.current.uniforms.tSkyLighting.value = toggles.skyLighting
        ? 1.0
        : 0.0;
      materialRef.current.uniforms.tCircuitOverlay.value =
        toggles.circuitOverlay ? 1.0 : 0.0;
      materialRef.current.uniforms.tIntersectionGlow.value =
        toggles.intersectionGlow ? 1.0 : 0.0;
...
export default function ShaderBackground({
  ...
  return (
    <div
      className={`fixed inset-0 w-full h-full pointer-events-none z-0 overflow-hidden transition-colors duration-700 ${isDarkMode ? "bg-[#0a0f1c]" : "bg-white"}`}
    >
